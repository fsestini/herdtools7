...
let partitions_of_cat (ins : AST.ins list) : (string * bool) list list =
    ...
    try
      ...
      let solution =
        Hashtbl.fold
          ...
          symbols_map []
      in
      Solver.exclude solver (List.map snd solution);
      let solution = List.map (fun (v, (_, b)) -> (v, b)) solution in
      loop (solution :: acc) ()
