module Log = (val Logs.src_log (Logs.Src.create "lasso") : Logs.LOG)
open Herdlib
...
module Make
    (S : SemExtra.S)
    (W : Weight.S)
    (WR : WeightedRel.S with type weight = W.t and type elt = S.E.event) =
struct
  module E = S.E
  ...
  let find_static_loop_boundaries ~(cutoff : E.event) (es : E.event_structure) =
    ...
    in
    let cutoff_proc = iiid.proc in
    let start_spoi = iiid.static_poi in
    let cutoff_poi = iiid.program_order_index in
    let bcc_events =
      events
  ...
  let build_lasso es cutoff ~rf_reg ~rf ~co ~po =
    ...
    let iterations = iterations_of_loop loop es.events in
